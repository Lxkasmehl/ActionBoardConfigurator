...
const ColumnFormFields = ({
  ...
  const [selectedCurrentEntityProp, setSelectedCurrentEntityProp] =
    useState('');
  const [showCustomRelationModal, setShowCustomRelationModal] = useState(false);
  ...
  useEffect(() => {
    const currentEntity =
      isIframeValidationError && columnData
        ? columnData.entity
        : editedItem.entity;
    if (currentEntity && mainEntity && currentEntity.name !== mainEntity.name) {
      const mainEntityProps = mainEntity.properties?.navigationProperties || [];
